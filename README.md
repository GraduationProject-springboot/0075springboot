...
# 3系统分析
## 3.1可行性分析
通过对本基于web的智慧社区设计与实现实行的目的初步调查和分析，提出可行性方案并对其一一进行论证。我们在这里主要从技术可行性、经济可行性、操作可行性等方面进行分析。
### 3.1.1技术可行性
本基于web的智慧社区设计与实现采用SSM框架，JAVA作为开发语言，是基于WEB平台的B/S架构系统。
...
因此，基于web的智慧社区设计与实现在开发技术上具有很高可行性，且开发人员掌握了一定的开发技术，所以此系统的开发技术具有可行性。
### 3.1.2经济可行性
本基于web的智慧社区设计与实现采用的软件都是开源的，这样能够削减很多的精力和资源，降低开发成本。同时对计算机的配置要求也极低，即使是淘汰下来的计算机也能够满足需要，因此，本系统在经济上是完全具有可行性的，所以在经济上是十分可行的。
### 3.1.3操作可行性
本基于web的智慧社区设计与实现的界面简单易操作，用户只要平时有在用过电脑，都能进行访问和操作。本系统具有易操作、易管理、交互性好的特点，在操作上是非常简单的，因此在操作上具有很高的可行性。

综上所述，此系统开发目标已明确，在技术、经济和操作方面都具有很高的可行性，并且投入少、功能完善、管理方便，因此系统的开发是完全可行的。
## 3.2系统性能分析
### 3.2.1 系统安全性
此基于web的智慧社区设计与实现要严格控制管理权限，具体要求如下：

（1）要想对基于web的智慧社区设计与实现进行管理，首先要依靠用户名和密码在系统中登陆，无权限的用户不可以通过任何方式登录系统和对系统的任何信息和数据进行查看，这样可以保证系统的安全可靠性和准确性。
...
|shentizhuangkuang|longtext|是|NULL|身体状况|
|yanglaojinfafang|varchar(200)|是|NULL|养老金发放|
...
|timushu|varchar(200)|是|NULL|题目数|
...
|yonghuming|varchar(200)|是|NULL|用户名|
|xingming|varchar(200)|是|NULL|姓名|
|dajuanneirong|longtext|是|NULL|答卷内容|
|dajuan|varchar(200)|是|NULL|答卷|
表4.3 车位信息
...
|cheweileibie|varchar(200)|是|NULL|车位类别|
|cheweitupian|varchar(200)|是|NULL|车位图片|
|cheweiweizhi|varchar(200)|是|NULL|车位位置|
|cheweizhuangtai|varchar(200)|是|NULL|车位状态|
|cheweijiage|int(11)|否||车位价格|
|cheweixiangqing|longtext|是|NULL|车位详情|
表4.4 车位租买
...
|cheweiweizhi|varchar(200)|是|NULL|车位位置|
|cheweizhuangtai|varchar(200)|是|NULL|车位状态|
|cheweijiage|varchar(200)|否||车位价格|
|cheweixiangqing|varchar(200)|是|NULL|车位详情|
|yonghuming|varchar(200)|是|NULL|用户名|
|xingming|varchar(200)|是|NULL|姓名|
...
|jiatingzhuzhi|longtext|是|NULL|家庭住址|
|jiaofeiqingkuang|longtext|是|NULL|缴费情况|
|fangwuxinxi|longtext|是|NULL|房屋信息|
|fangchanziliao|longtext|是|NULL|房产资料|
|zhaopian|varchar(200)|是|NULL|照片|
表4.9 留言板
...
|id (主键)|bigint(20)|否||主键|
|addtime|timestamp|否|CURRENT\_TIMESTAMP|创建时间|
|baoxiudanhao|varchar(200)|是|NULL|报修单号|
|baoxiumingcheng|varchar(200)|是|NULL|报修名称|
|baoxiutupian|varchar(200)|是|NULL|报修图片|
|menpaihao|varchar(200)|否||门牌号|
|baoxiuneirong|longtext|是|NULL|报修内容|
|baoxiuriqi|date|是|NULL|报修日期|
|baoxiubeizhu|varchar(200)|是|NULL|报修备注|
|yonghuming|varchar(200)|是|NULL|用户名|
|xingming|varchar(200)|是|NULL|姓名|
|shouji|varchar(200)|是|NULL|手机|
|userid|bigint(20)|是|NULL|用户id|
...
|id (主键)|bigint(20)|否||主键|
|addtime|timestamp|否|CURRENT\_TIMESTAMP|创建时间|
|fuwumingcheng|varchar(200)|否||服务名称|
|fuwuleixing|varchar(200)|否||服务类型|
|fuwutupian|varchar(200)|是|NULL|服务图片|
|fuwuneirong|longtext|是|NULL|服务内容|
|fuwujiage|int(11)|否||服务价格|
|fuwujieshao|longtext|是|NULL|服务介绍|
